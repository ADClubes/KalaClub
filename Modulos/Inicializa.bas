...
Global sTicket As String                             '  Impresora de Tickets
...
Global ChangePassword As Boolean        ' Variable para controlar el cambio de password

'27/05/08
Global iDB_IdUser As Integer            'Id del usuario
...
'19/10/2011
Global bDireccionado As Boolean 'Indica si se utilizan las nuevas funciones de direccionados.
'13/12/2011
Global bSeguroVida As Boolean  'Indica al sistema el uso de los controles de seguro de usuario.
...
Public Function Connection_DB() As Boolean
    ...
    ' Inicializa Variables de conexion a la base de datos NITGEN
'    Set objAccessManager = New AccessManager
...
'    Set objTerminalInfo = objAccessManager.TerminalInfo
    'Set objTerminalList = New ITerminalList
    #If SqlServer_ Then
        ...
        '''''''''''''''''''
        ''SAN ANGEL
        'strConn = "DRIVER=SQL Server;SERVER=172.16.2.1;DATABASE=KalaclubSQL;UID=sa;PWD=password;"
        ''
        ''COYOACAN
        'strConn = "DRIVER=SQL Server;SERVER=172.16.1.5;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        ''
        ''DESIERTO
        'strConn = "DRIVER=SQL Server;SERVER=172.16.3.1;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        ''
        ''ARBOLEDAS
        'strConn = "DRIVER=SQL Server;SERVER=172.16.5.1;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        ''
        ''SANTA FE
        'strConn = "DRIVER=SQL Server;SERVER=172.16.4.1;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        ''
        ''LOMAS VERDES
        'strConn = "DRIVER=SQL Server;SERVER=172.16.6.1;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        ''
        ''CUAUTITLAN IZCALLI
        'strConn = "DRIVER=SQL Server;SERVER=172.16.7.1;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        ...
        '''''''''''''''''''
        ''SERVIDOR 2.5
        'strConn = "DRIVER=SQL Server;SERVER=172.16.2.5;DATABASE=KalaclubSQL;UID=sa;PWD=passwordsql123;"
        
        ''Efren Valdes 13/03/13
        ''Extrae los datos del servidor y base de datos del archivo KalaClub.ini''
        strConn = "DRIVER=SQL Server;SERVER=" + sDB_DataSource + ";Connection Timeout=5000;DATABASE=" + sDB + _
                 ";UID=SportiumSQL;PWD=Sportium123+"
        '         strConn = "DSN=KalaDB;DATABASE=" + sDB + _
                  ";UID=SportiumSQL;PWD=Sportium123+"
        ''
        ''VM WARE
        'strConn = "DRIVER=SQL Server;SERVER=172.16.2.5;DATABASE=KalaclubSQL;UID=sa;PWD=Passw0rd;"
        '
        ''HOST COMO SERVIDOR
        'strConn = "DRIVER=SQL Server;SERVER=192.168.0.1;DATABASE=KalaclubSQL;UID=sa;PWD=password123;"
        '
        'strConn = "DRIVER=SQL Server;SERVER=(local);DATABASE=KalaclubSQL;UID=sa;PWD=password123;"

        'strTorniquetes = "127.0.0.1"
...
Public Function Lee_Ini() As Byte
    ...
    IniString = String(255, " ")
    Regresa = GetPrivateProfileStringA("PARAMETROS", "TICKET", " ", IniString, Len(IniString), ArchivoIni)
'    If Regresa = 0 Then
'        Mensaje = Mensaje + "TICKET" + Chr(10) + Chr(13)
'    End If
    sTicket = Trim(IniString)
    If Asc(Right(sTicket, 1)) = 32 Or Asc(Right(sTicket, 1)) = 0 Then
        sTicket = Left(sTicket, Len(Trim(sTicket)) - 1)
    End If
    ...
    '--------------------------------------------------------------------
    '19/10/2011
    'Indicador funciones direccionados
    '--------------------------------------------------------------------
    #If SqlServer_ Then
    #Else
        IniString = String(255, " ")
        Regresa = GetPrivateProfileStringA("PARAMETROS", "DIRECCIONADOS", " ", IniString, Len(IniString), ArchivoIni)
        IniString = Trim(IniString)
        If Asc(Right(IniString, 1)) = 32 Or Asc(Right(IniString, 1)) = 0 Then
            IniString = Left(IniString, Len(Trim(IniString)) - 1)
        End If
...
Sub LlenaCombos(cboControl As Control, strQuery, strCampo, strCampoAlt As String)
   ...
   Loop
End Sub
Sub LlenaComboSP(cboControl As Control, strSP, strCampo, strCampoAlt, strCampo3 As String)
    Dim AdoRcsCombos As ADODB.Recordset
    Dim cmd As ADODB.Command
    ...
    Set cmd = New ADODB.Command
             ...
             cmd.Parameters.Append cmd.CreateParameter("Id", adVarChar, adParamInput, 6, strCampo3)
            
             Set AdoRcsCombos = cmd.Execute
...
Sub EliminaAccionista(lngExAccionista As Long)
    ...
    #If SqlServer_ Then
        strSQL = "INSERT INTO exaccionistas (fecha_alta, a_paterno, a_materno, " & _
                    ...
                    "VALUES ('" & Format(AdoRcsExAcc!fecha_alta, "yyyymmdd") & "', '" & _
    ...
    #Else
        strSQL = "INSERT INTO exaccionistas (fecha_alta, a_paterno, a_materno, " & _
                    ...
                    "telefono_2, empresa, e_calle, e_colonia, e_ent_federativa, " & _
                    "e_delegamunici, e_telefono_1, e_telefono_2, fecha_baja) " & _
                    "VALUES (#" & Format(AdoRcsExAcc!fecha_alta, "mm/dd/yyyy") & "#, '" & _
                    AdoRcsExAcc!A_Paterno & "', '" & AdoRcsExAcc!A_Materno & "', '" & _
                    AdoRcsExAcc!Nombre & "', '" & AdoRcsExAcc!calle & "', '" & _
                    AdoRcsExAcc!colonia & "', '" & AdoRcsExAcc!ent_federativa & "', '" & _
                    AdoRcsExAcc!delegamunici & "', '" & AdoRcsExAcc!telefono_1 & "', '" & _
...
'20111220 UCM
Public Function NombreVendedor(ByVal pIdVendedor As Integer) As String
    Dim strSQL As String
    Dim rsVendedor As ADODB.Recordset
    
    strSQL = "SELECT IdUsuario,Login_Name,uPassword,Nombre,Fecha_Alta,IdPerfil,FechaVencePass,Status FROM USUARIOS_SISTEMA WHERE IdUsuario = " & CStr(pIdVendedor)
    
    Set rsVendedor = New ADODB.Recordset
    
    rsVendedor.CursorLocation = adUseServer
    
    rsVendedor.Open strSQL, Conn, adOpenForwardOnly, adLockReadOnly
    
    If Not rsVendedor.EOF Then
        NombreVendedor = Trim(rsVendedor!Nombre)
    End If
