...
Begin VB.Form frmFacInsFormaPago 
...
Private Sub cmdAceptar_Click()
    ...
    If Me.txtNoCuenta.Visible And Len(Me.txtNoCuenta.Text) < 4 Then
        If iNumeroCaja <> 2 Then
            MsgBox "Indicar al menos los 4 últimos digitos de la cuenta de pago", vbExclamation, "Verifique"
    ...
    If iModo = 0 Then
        ...
        'Cadena original
        'Me.sscmbFormaPago.Columns("IdForma").Value & vbTab & Me.sscmbFormaPago.Columns("Descripcion").Value & vbTab & Me.ssCmbOpcionPago.Columns("Descripcion").Value & vbTab & Me.txtImporteaCobrar.Text & vbTab & vbNullString & vbTab & Me.txtImporteaCobrar.Text & vbTab & Me.ssCmbOpcionPago.Columns("IdOpcion").Value & vbTab & Trim(Me.txtLote.Text) & vbTab & Trim(Me.txtOperacion.Text) & vbTab & Me.txtImpRecibido.Text & vbTab & Me.dtpFechaOpera.Value
        
        
        sCadPago = Me.ssCmbFormaPago.Columns("IdForma").Value & vbTab & Me.ssCmbFormaPago.Columns("Descripcion").Value & vbTab
        
        If Me.ssCmbOpcionPago.Visible Then
            sCadPago = sCadPago & Me.ssCmbOpcionPago.Columns("Descripcion").Value & vbTab
        Else
            sCadPago = sCadPago & "" & vbTab
        End If
        
        sCadPago = sCadPago & Me.txtImporteaCobrar.Text & vbTab & vbNullString & vbTab & Me.txtImporteaCobrar.Text & vbTab
        
        
        If Me.ssCmbOpcionPago.Visible Then
            sCadPago = sCadPago & Me.ssCmbOpcionPago.Columns("IdOpcion").Value & vbTab
        Else
            sCadPago = sCadPago & 0 & vbTab
        End If
        
        If Me.txtLote.Visible Then
            sCadPago = sCadPago & Trim(Me.txtLote.Text) & vbTab & Trim(Me.txtOperacion.Text)
        Else
            sCadPago = sCadPago & vbNullString & vbTab & vbNullString
        End If
        
        sCadPago = sCadPago & vbTab & Me.txtImpRecibido.Text & vbTab & Me.dtpFechaOpera.Value
        
        If Me.txtNoCuenta.Visible = True And Me.txtNoCuenta <> vbNullString Then
            sCadPago = sCadPago & vbTab & Me.txtNoCuenta.Text
        Else
            sCadPago = sCadPago & vbTab & vbNullString
        End If
        
    
        frmFacturacion.ssdbgPagos.AddItem sCadPago
...
Private Sub Form_Load()
    ...
    #If SqlServer_ Then
        strSQL = "SELECT Descripcion, IdFormaPago, CONVERT(int, ISNULL(TieneOpcion,0)) AS TieneOpcion, CONVERT(int, ISNULL(TieneTPV,0)) AS TieneTPV, CONVERT(int, ISNULL(TieneLote,0)) AS TieneLote, CONVERT(int, ISNULL(TieneOperacion,0)) AS TieneOperacion"
    #Else
